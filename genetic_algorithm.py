...
class GeneticAlgorithm:
	def __init__(self, use_multiprocessing=False):
		...
		print("Use multiproccesing: {}".format(self.use_multiprocessing))
		...
		for gen in range(self.n_iter):
			...
			if self.use_multiprocessing:
				with Pool(os.cpu_count()-1) as pool:
					scores = pool.map(self.objective, [d for d in decoded])
					# Scores should get listed in the correct order, since map is 
					# synchronous and waits for all threads to finish
			else:
